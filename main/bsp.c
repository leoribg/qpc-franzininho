...
void BSP_init(void)   {
    ESP_LOGI(TAG, "Simple Blinky example\n"
           "QP/C version: %s\n"
           "Press Ctrl-C to quit...\n",
           QP_VERSION_STR);
}
void BSP_ledOff(void) { ESP_LOGI(TAG, "LED OFF\n"); }
void BSP_ledOn(void)  { ESP_LOGI(TAG, "LED ON\n");  }
...
void Q_onAssert(char const * const module, int loc) {
    ESP_LOGE(TAG, "Assertion failed in %s:%d", module, loc);
    exit(-1);
}